...
describe('Trip Search API (User Flow)', () => {
    ...
    beforeAll(async () => {
        ...
        const timeVip = new Date(testDate);
        timeVip.setHours(8, 0, 0, 0);
        ...
        const timeNormal = new Date(testDate);
        timeNormal.setHours(10, 0, 0, 0);
        ...
        for (let i = 1; i <= 5; i++) {
            await pool.query(
                `INSERT INTO bookings (trip_id, passenger_name, passenger_phone, seat_number, total_price, booking_code, contact_email) VALUES ($1, 'Test', '000', $2, 0, $3, 'test@example.com')`,
                [tripVipId, `A${i}`, `VIP-${i}`],
            );
        }
        await pool.query(
            `INSERT INTO bookings (trip_id, passenger_name, passenger_phone, seat_number, total_price, booking_code, contact_email) VALUES ($1, 'Test', '000', 'B1', 0, 'NOR-1', 'test@example.com')`,
            [tripNormalId],
        );
    });
    ...
    it('3. Should calculate available_seats correctly', async () => {
        ...
        const normalTrip = res.body.data.find(
            (t) => t.trip_id === tripNormalId,
        );
    ...
    it('4. Should FILTER by Price Range (Min/Max)', async () => {
        ...
        expect(resCheap.body.data.some((t) => t.trip_id === tripVipId)).toBe(
            false,
        );
    });
    ...
    it('7. Should SORT by Price ASC', async () => {
        ...
        if (data.length >= 2) {
            expect(parseFloat(data[0].price_base)).toBeLessThanOrEqual(
                parseFloat(data[1].price_base),
            );
    ...
    it('8. Should hide Full Trips (0 available seats)', async () => {
        ...
        for (let i = 2; i <= 40; i++) {
            promises.push(
                pool.query(
                    `INSERT INTO bookings (trip_id, passenger_name, passenger_phone, seat_number, total_price, booking_code, contact_email) VALUES ($1, 'Full', '000', $2, 0, $3, 'full@example.com')`,
                    [tripNormalId, `B${i}`, `FULL-${i}`],
                ),
            );
        }
    ...
    afterAll(async () => {
        await pool.query(
            'DELETE FROM bookings WHERE passenger_name IN ($1, $2)',
            ['Test', 'Full'],
        );
        if (tripVipId && tripNormalId) {
            await pool.query('DELETE FROM trips WHERE id IN ($1, $2)', [
                tripVipId,
                tripNormalId,
            ]);
        }
        if (routeVipId && routeNormalId) {
            await pool.query('DELETE FROM routes WHERE id IN ($1, $2)', [
                routeVipId,
                routeNormalId,
            ]);
        }
        if (busVipId && busNormalId) {
            await pool.query('DELETE FROM buses WHERE id IN ($1, $2)', [
                busVipId,
                busNormalId,
            ]);
        }
